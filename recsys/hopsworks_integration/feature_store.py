...
def create_customers_feature_group(fs, df, online_enabled=True):
...
def create_articles_feature_group(
    fs,
    df,
    articles_description_embedding_dim,
    online_enabled=True,
):
...
def create_transactions_feature_group(fs, df, online_enabled=True):
...
def create_interactions_feature_group(fs, df, online_enabled=True):
...
def create_ranking_feature_group(fs, df, parents, online_enabled=True):
...
def create_ranking_feature_views(fs):
    ...
    # Select features
    selected_features_ranking = rank_fg.select_except(
        ["customer_id", "article_id"]
    ).join(trans_fg.select(["month_sin", "month_cos"]))
